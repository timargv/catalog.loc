                    ...
                    <div class="d-flex">
                        <ul class="list-menu list-unstyled n-navigation-vertical-category py-4">
                            @foreach($menuRoot as $category)
                                <li id="aHref" data-id="{{ $category->id }}" title="{{ $category->name }}"><a href="{{ route('categories.show', $category->slug?:$category->id) }}"><span>{{ $category->name }}</span></a></li>
                            @endforeach
                        </ul>
                                        ...
                                        @foreach($categories->children as $category)
                                            <div class="pb-1 card border-0 py-0 px-0">
                                                <div class="vertical-menu-block">
                                                    <div class="vertical-menu-block-title font-weight-bold"><a href="{{ route('categories.show', $category->slug?:$category->id) }}">{{ $category->name }}</a></div>
                                                    <ul class="vertical-menu-block-list-item list-unstyled">
                                                        @foreach($category->children as $child)
                                                            <li><a href="{{ route('categories.show', $child->slug?:$child->id) }}">{{ $child->name }}</a></li>
                                                        @endforeach
                                                    </ul>
                                                </div>
                                            </div>
